...
load_item() {
  ...
  local item
  item=$(gh project item-list 3 --owner AllAboard-THP --limit 50 --format json | jq -r --arg u "$url" '.items[] | select(.content.url==$u) | .id')
  ITEM_ID[$n]=$item
}

set_vague() { gh project item-edit --id "${ITEM_ID[$1]}" --project-id "$PROJECT_ID" --field-id "$FIELD_VAGUE" --single-select-option-id "$2" >/dev/null; }
set_start() { gh project item-edit --id "${ITEM_ID[$1]}" --project-id "$PROJECT_ID" --field-id "$FIELD_START" --date "$2" >/dev/null; }
set_end()   { gh project item-edit --id "${ITEM_ID[$1]}" --project-id "$PROJECT_ID" --field-id "$FIELD_END" --date "$2" >/dev/null; }
...
echo "=== Vagues + dates roadmap (chevauchements = parallèle) ==="
# V1 — même date début = barres parallèles sur la Roadmap
for n in 18 24 33 31 32; do set_vague "$n" "$V1"; set_start "$n" "2026-05-14"; done
set_end 18 "2026-05-17"
set_end 33 "2026-05-18"
set_end 31 "2026-05-20"
set_end 32 "2026-05-20"
set_end 24 "2026-05-22"
...
# V3 — UI en parallèle dès V1 partiel (25 après 24 soft)
set_vague 25 "$V3"; set_start 25 "2026-05-18"; set_end 25 "2026-05-25"
