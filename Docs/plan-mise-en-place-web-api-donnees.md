...
| 2026-05-20 | Dokploy **dev** (Phase 2) | Vars API (`DATABASE_URL`, `JWT_SECRET`, `MVP_LOGIN_PASSWORD`, `PORT`, `NODE_ENV`) ; redeploy API ; `pnpm smoke:dev` OK ; `/help/new` + feed SSR (`https://dev.allaboard.fr`) OK. Runbook : [runbook-dokploy-dev-phase2.md](runbook-dokploy-dev-phase2.md). ADR implémentée : [0001](adr/0001-authentication-strategy.md) — revue équipe #18 en attente. |
